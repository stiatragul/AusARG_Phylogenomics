...
def run_blat(args, query):
	...
	subprocess.call("singularity exec ucsc-blat_366--hdd26221_0.sif blat %s %s %s -out=blast8" % (args.db,
				query, outfile1), shell=True)	

	# database to query
	outfile2 = os.path.join(subdir, 'probes_to_%s' % args.sample)
	#subprocess.call("%s %s %s %s -out=blast8" % (args.blat, query,
	subprocess.call("singularity exec ucsc-blat_366--hdd26221_0.sif blat %s %s %s -out=blast8" % (query,
