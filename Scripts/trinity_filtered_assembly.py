...
def get_reads(args):
	...
	if args.read1 == None and args.read2 == None:
		# no she didn't
		read1 = os.path.join(args.dir, 'trim_reads', '%s_R1_bb.final.fq.gz' % args.sample)
		read2 = os.path.join(args.dir, 'trim_reads', '%s_R2_bb.final.fq.gz' % args.sample)
...
def run_trinity(args, read1, read2):
	...
	if args.normal == False:
		...
		subprocess.call("singularity exec -e trinityrnaseq.v2.12.0.simg Trinity --seqType fq --max_memory %sG --left %s --right %s --CPU %s --output %s --NO_SEQTK" %
				(args.mem, read1, read2, cpus, subdir), shell=True)
		#consider using the "--bflyCalculateCPU" option which allows Trinity to use 80% of available memory in 10gb instances
	else:
		#subprocess.call("%s --normalize_reads --seqType fq --max_memory %sG --left %s --right %s --CPU %s --output %s" %
		#						(args.trinity, args.mem, read1, read2, cpus, subdir), shell=True)
		subprocess.call("%s --normalize_reads --seqType fq --max_memory %sG --left %s --right %s --CPU %s --output %s --NO_SEQTK" %
				(args.mem, read1, read2, cpus, subdir), shell=True)
