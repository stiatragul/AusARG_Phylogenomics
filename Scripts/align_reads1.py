...
def prepare_seq(args, genome):
	...
	if not os.path.isfile(genome + '.bwt'):
		...
		subprocess.call("singularity exec bwa_0.7.17--h5bf99c6_8.sif bwa index %s" % (genome), shell=True)
	if not os.path.isfile(genome + '.fai'):
		#subprocess.call("%s faidx %s" % (args.samtools, genome), shell=True)
		subprocess.call("singularity exec samtools_1.3.1--h1b8c3c0_8.sif samtools faidx %s" % (genome), shell=True)
	out = re.sub('.fa.*$', '.dict', genome)
	if not os.path.isfile(out):
		#subprocess.call("%s CreateSequenceDictionary R=%s O=%s" % 
		#						(args.picard, genome, out), shell=True)
		subprocess.call("singularity exec picard_2.25.5--hdfd78af_0.sif picard CreateSequenceDictionary R=%s O=%s" %
								(genome, out), shell=True)
...
def align_seq(args, r, seq, dir):
	...
	subprocess.call("singularity exec bwa_0.7.17--h5bf99c6_8.sif bwa mem -t %s %s %s %s > %s" % (args.CPU, seq, r[0], r[1], out1a), shell=True)
	#subprocess.call("%s mem -t %s %s %s > %s" % (args.bwa, args.CPU, seq, r[2], out1b), shell=True)
	subprocess.call("singularity exec bwa_0.7.17--h5bf99c6_8.sif bwa mem -t %s %s %s > %s" % (args.CPU, seq, r[2], out1b), shell=True)
	# fixmate
	...
	subprocess.call("singularity exec samtools_1.3.1--h1b8c3c0_8.sif samtools view -@ %s -b %s > %s" % (args.CPU, out1a, out1as), shell=True)
	#subprocess.call("%s FixMateInformation I=%s O=%s" % (args.picard, out1as, out2a), shell=True)
	subprocess.call("singularity exec picard_2.25.5--hdfd78af_0.sif picard FixMateInformation I=%s O=%s" % (out1as, out2a), shell=True)
	#subprocess.call("%s view -@ %s -b %s > %s" % (args.samtools, args.CPU,  out1b, out2b), shell=True)
	subprocess.call("singularity exec samtools_1.3.1--h1b8c3c0_8.sif samtools view -@ %s -b %s > %s" % (args.CPU,  out1b, out2b), shell=True)
	# sorted
	#subprocess.call("%s sort -@ %s -O bam -o %s -T %s %s" % (args.samtools, args.CPU, out3a, tmpdir, out2a), shell=True)
	subprocess.call("singularity exec samtools_1.3.1--h1b8c3c0_8.sif samtools sort -@ %s -O bam -o %s -T %s %s" % (args.CPU, out3a, tmpdir, out2a), shell=True)
	#subprocess.call("%s sort -@ %s -O bam -o %s -T %s %s" % (args.samtools, args.CPU, out3b, tmpdir, out1b), shell=True)
	subprocess.call("singularity exec samtools_1.3.1--h1b8c3c0_8.sif samtools sort -@ %s -O bam -o %s -T %s %s" % (args.CPU, out3b, tmpdir, out1b), shell=True)
	# readgroup
	...
	subprocess.call("singularity exec picard_2.25.5--hdfd78af_0.sif picard AddOrReplaceReadGroups INPUT=%s OUTPUT=%s RGLB=%s RGPL=Illumina RGPU=%s RGSM=%s" % 
						  (out3a, out4a, args.sample, args.sample, args.sample), shell=True)
	#subprocess.call("%s AddOrReplaceReadGroups INPUT=%s OUTPUT=%s RGLB=%s RGPL=Illumina RGPU=%s RGSM=%s" % 
	#					  (args.picard, out3b, out4b, args.sample, args.sample, args.sample), shell=True)
	subprocess.call("singularity exec picard_2.25.5--hdfd78af_0.sif picard AddOrReplaceReadGroups INPUT=%s OUTPUT=%s RGLB=%s RGPL=Illumina RGPU=%s RGSM=%s" %
						  (out3b, out4b, args.sample, args.sample, args.sample), shell=True)
	...
	subprocess.call("singularity exec picard_2.25.5--hdfd78af_0.sif picard MarkDuplicates I=%s O=%s ASO=coordinate METRICS_FILE=%s" %
					   (out4a, out5a, m_1), shell=True)
	#subprocess.call("%s MarkDuplicates I=%s O=%s ASO=coordinate METRICS_FILE=%s" % 
	#				   (args.picard, out4b, out5b, m_2), shell=True)
	subprocess.call("singularity exec picard_2.25.5--hdfd78af_0.sif picard MarkDuplicates I=%s O=%s ASO=coordinate METRICS_FILE=%s" %
					   (out4b, out5b, m_2), shell=True)
	#subprocess.call("%s merge -@ %s %s %s %s" % (args.samtools, args.CPU, out6, out5a, out5b), shell=True)
	subprocess.call("singularity exec samtools_1.3.1--h1b8c3c0_8.sif samtools merge -@ %s %s %s %s" % (args.CPU, out6, out5a, out5b), shell=True)
	# indel target
	#subprocess.call("%s index %s" % (args.samtools, out6), shell=True)
	subprocess.call("singularity exec samtools_1.3.1--h1b8c3c0_8.sif samtools index %s" % (out6), shell=True)
